...
/// <summary>
/// Serializes acoustic speech-analysis results to
...
/// is the primary acoustic overload. It uses real per-frame measurements from
/// <see cref="Phonematic.Services.AcousticFeatureExtractorService"/> to populate all
/// prosodic fields of every <c>&lt;phon&gt;</c> element.
/// </para>
/// <para>
/// The Whisper-based legacy writer lives in <c>Phonematic.Gui</c> as
/// <c>PhoScriptWriterLegacy</c>; it is retained only for the GUI's legacy backend and
/// pulls in the CMU/G2P/IPA helpers that the acoustic path does not need.
/// </para>
/// <para>
/// All XML emission goes through <see cref="XmlWriter"/> (from <c>System.Xml</c>), which
/// guarantees well-formed output and correct attribute-value escaping without any
...
public static class PhoScriptWriter
{
        ...
        DateOnly? recordedDate = null)
    {
        ...
        using (var xw = XmlWriter.Create(sw, XmlSettings))
        {
            ...
            WriteMeta(xw, Path.GetFileNameWithoutExtension(sourceFileName), date, "wav2vec2-phoneme");
            WriteSpeaker(xw, baseline, speakerId);

            var words = GroupPhonesIntoWords(phones);
            for (var w = 0; w < words.Count; w++)
            {
                var boundary = w == words.Count - 1 ? "IP_end" : "none";
                WriteWord(xw, string.Empty, words[w], boundary, frames, baseline);
            }
    ...
    // ------------------------------------------------------------------
    // Word-aware overload (forced-alignment / Whisper-hybrid paths)
    // ------------------------------------------------------------------

    /// <summary>
    /// Builds a PhoScript document from time-aligned words grouped into sentences. Each inner list
    /// becomes one <c>&lt;sentence&gt;</c>, and <see cref="WordAlignment.Orth"/> is written verbatim
    /// as the word's <c>orth</c> — so the output reflects the actual words spoken. Phone-level
    /// prosody is computed from <paramref name="frames"/> exactly as in the free-decode overload.
    /// </summary>
    /// <param name="sentences">Sentences, each a list of time-aligned words (e.g. one per Whisper segment).</param>
    /// <param name="frames">Per-frame acoustic feature measurements (whole utterance).</param>
    /// <param name="baseline">Speaker-level statistics used for relative calculations.</param>
    /// <param name="sourceFileName">Base name of the source audio file.</param>
    /// <param name="asrModel">Value written to <c>meta/@asr_model</c> (e.g. <c>"wav2vec2-phoneme+forced-align"</c>).</param>
    /// <param name="speakerId">Optional speaker identifier written into each <c>&lt;speaker&gt;</c> block.</param>
    /// <param name="recordedDate">Recording date; defaults to today.</param>
    public static string Write(
        IReadOnlyList<IReadOnlyList<WordAlignment>> sentences,
        IReadOnlyList<AcousticFeatureFrame> frames,
        SpeakerBaseline baseline,
        string sourceFileName,
        string asrModel,
        ...
        DateOnly? recordedDate = null)
    {
        ...
        using (var xw = XmlWriter.Create(sw, XmlSettings))
        {
            for (var i = 0; i < sentences.Count; i++)
            {
                ...
                var start = words.Count > 0 ? words[0].TStartMs : 0;
                var end = words.Count > 0 ? words[^1].TEndMs : 0;

                xw.WriteStartElement("sentence");
                xw.WriteAttributeString("id", $"utt_{i + 1:D3}");
                xw.WriteAttributeString("lang", "en-US");
                xw.WriteAttributeString("duration_ms", (end - start).ToString());

                WriteMeta(xw, recordingId, date, asrModel);
                WriteSpeaker(xw, baseline, speakerId);

                for (var w = 0; w < words.Count; w++)
                {
                    var boundary = w == words.Count - 1 ? "IP_end" : "none";
                    WriteWord(xw, words[w].Orth, words[w].Phones, boundary, frames, baseline);
                }
    ...
    /// <summary>Writes the <c>&lt;meta&gt;</c> element shared by every sentence.</summary>
    private static void WriteMeta(XmlWriter xw, string recordingId, DateOnly date, string asrModel)
    {
        ...
        xw.WriteAttributeString("asr_model", asrModel);
        xw.WriteEndElement(); // </meta>
    }

    /// <summary>Writes the <c>&lt;speaker&gt;</c> baseline block.</summary>
    private static void WriteSpeaker(XmlWriter xw, SpeakerBaseline baseline, string speakerId)
    {
    ...
    /// <summary>
    /// Writes a single <c>&lt;word&gt;</c> element and its child <c>&lt;phon&gt;</c> elements.
    /// Used by both the free-decode (empty <paramref name="orth"/>) and word-aware paths.
    /// </summary>
        ...
        SpeakerBaseline baseline)
    {
        ...
        xw.WriteAttributeString("orth", orth);
        xw.WriteAttributeString("t_start", (phones.Count > 0 ? phones[0].TStartMs : 0).ToString());
        xw.WriteAttributeString("t_end", (phones.Count > 0 ? phones[^1].TEndMs : 0).ToString());
        xw.WriteAttributeString("phrase_boundary", boundary);

        for (var p = 0; p < phones.Count; p++)
        {
            WritePhon(xw, phones[p], frames, baseline,
                prevPhone: p > 0 ? phones[p - 1] : null,
                nextPhone: p < phones.Count - 1 ? phones[p + 1] : null);
        }
